...
Текущие значения по умолчанию в чартах и `docker-compose.yml` — `0.33` для
компонентов Hub (backend, worker, frontend, iac-scanner) и `0.32` для
DomainScope: тега `0.33` у образа `dexionius/domain-scope` нет, компоненты

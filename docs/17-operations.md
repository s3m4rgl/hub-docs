...
### В Kubernetes выключайте файловый лог-синк

> **`LOG_FILE_SINK_ENABLED=false` — для k8s это не тонкая настройка, а необходимость.**

Помимо stdout Hub по умолчанию пишет логи ещё и в файл (`logs/app.log`, `logs/error.log`) с ротацией. В контейнере этот каталог обычно лежит на смонтированном томе, владелец которого **не совпадает** с non-root пользователем контейнера. Тогда первая же ротация падает:

```
write error: can't rename log file: rename logs/app.log logs/app-...log: permission denied
```

Дальше эта строка печатается **на каждую запись в лог**. Замер на рабочем стенде: 78 таких строк за 3 минуты в воркере — шум удваивает объём логов и топит в себе настоящие сообщения.

Хуже, что это **тихий отказ**: stdout рядом продолжает работать, снаружи всё выглядит нормально, и файловый синк может месяцами не писать ничего — пока кто-нибудь не полезет за файлом и не обнаружит, что его нет.

В k8s логи и так собираются со stdout, поэтому файловый синк здесь — чистый риск (права, диск, ротация) без пользы:
...
Для bare-metal и VM без централизованного сбора логов дефолт `true` правильный — там файл и есть основной канал.
...
Hub не управляет настройками сервера БД — их задаёт тот, кто разворачивает PostgreSQL. **Дефолты официального образа рассчитаны на «просто запустилось», а не на рабочий объём**, и на инсталляции с сотнями тысяч находок это становится главным источником тормозов.

Больнее всего `shared_buffers` с дефолтом **128 MB**. Для масштаба, замер на стенде с 471 тыс. находок: таблица `findings` — 583 MB кучи плюс около 1 GB индексов, очередь `river_job` — ещё 822 MB. То есть ~2,4 GB горячих данных обслуживались кэшем меньше десятой доли от них, cache hit ratio держался на **67%** при норме для OLTP выше 95%, и каждый третий блок читался с диска.
...
Разумная отправная точка — 25% от памяти, выделенной контейнеру БД:
...
- **`work_mem`** — на *каждый узел сортировки/хэша в каждом соединении*, а не на запрос. Считайте худший случай: `work_mem × соединения × узлы` не должно приближаться к лимиту памяти.
- **`maintenance_work_mem`** — скорость `VACUUM` и построения индексов.

Если том БД на **HDD** (в том числе сетевом), оставьте `random_page_cost=4` и `effective_io_concurrency=1`: их дефолты описывают именно вращающийся диск. Выставлять SSD-значения (`1.1` и `200`) без реального SSD значит соврать планировщику — он начнёт считать случайное чтение дешёвым и выбирать index-scan там, где выгоднее seq-scan.

> **Следствие для HDD-томов:** рестарт пода БД — дорогая операция, а не рутинная. Кэш обнуляется, и первые запросы читают всё с диска. На упомянутом стенде первый запрос после пересоздания пода занял **90 секунд** против ~1 секунды на прогретом кэше. Планируйте рестарты соответственно и не пугайтесь этой картины сразу после деплоя.

...
Начиная с 0.33 задачи заводит плагин, поэтому настройки ищите в его
конфигурации — установочной и попроектной, а не в переменных окружения.
См. [Официальные плагины](plugins-official.md) и
[Интеграцию с Jira](integration-jira.md).

### Кнопки заведения задачи нет

Провайдер не привязан к проекту либо плагин не включён. Установка сама по
себе плагин не запускает.

### Создание задачи: `401 Unauthorized`

- Jira Cloud: в поле токена должен быть **API-токен**, а не пароль учётной
  записи;
- Jira Data Center: истёк персональный токен;
- проверить вручную: `curl -u <учётная-запись>:<токен> <адрес-jira>/rest/api/2/myself`.

### Создание задачи: `403 Forbidden`

У учётной записи нет права создавать задачи в этом проекте. Проверьте
Project → Permissions в Jira.

### Переход по статусу отклонён

Цепочка переходов после создания ведёт по статусу, недоступному в схеме
работ Jira. Посмотрите Project → Workflow: какие переходы разрешены из
статуса, в котором задача оказывается сразу после создания.

### Опрос статусов отчитывается об успехе, но ничего не закрывает

Проверьте, заполнены ли учётные данные в конфигурации плагина **для этого
проекта**. Незаполненные дают отказ на каждом опросе; до 0.33 такой отказ
выглядел как «проверено, закрывать нечего».

### Закрытие задачи не закрывает находку

Начиная с 0.33 статус возвращается двумя путями — вебхуком от системы учёта
задач и опросом статусов со стороны Hub. Прежние переменные
`FEATURE_JIRA_REVERSE_SYNC` и `JIRA_REVERSE_SYNC_*` **удалены** и ни на что
не влияют.

| Признак | Что проверить |
| --- | --- |
| Вебхук не доходит | Настроен ли он на стороне Jira; совпадает ли секрет в адресе; достижим ли Hub из Jira |
| Ни вебхук, ни опрос ничего не меняют | Заполнены ли учётные данные в конфигурации плагина — незаполненные дают отказ на каждом опросе |
| Статус в Jira «закрыт», а находка открыта | Считается ли этот статус завершающим в настройках плагина |
| Обращение блокируется | Jira во внутренней сети — `JIRA_ALLOW_LOCAL_DIAL` вместе с `JIRA_BASE_URL_ALLOWLIST` |

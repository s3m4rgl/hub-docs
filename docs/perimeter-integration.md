...
Настройка разнесена по трём разным местам, и это главный источник путаницы.
Ниже — карта: что делается в веб-интерфейсе Hub, что в чарте, а что переменными
окружения самого воркера.

| Что настраиваем | Где именно | Чем |
| --- | --- | --- |
| Проект, продукты, сервисная учётная запись, ключ и права | **веб-интерфейс Hub** | мышкой; альтернативно — REST API |
| Границы сканирования: включения и исключения по доменам и подсетям | **веб-интерфейс Hub** → проект → Скоуп | вручную, импортом CSV или синхронизацией с NetBox |
| Решения по предложениям сканера | **веб-интерфейс Hub** → проект → Скоуп → Предложения | мышкой |
| Периодический импорт периметра из NetBox | **веб-интерфейс Hub** → проект → Скоуп → Синхронизации | форма задания синхронизации |
| Адреса Hub, идентификаторы проекта и продукта, ключ | **воркер** | переменные окружения `DOMAINSCOPE_*` |
| Какие сканеры включены и с какой периодичностью | **воркер** | переменные окружения `DOMAINSCOPE_*` |
| Куда попадают находки каждого движка | **воркер** | переменные `*_SARIF_PRODUCT_ID` |
| Развёртывание воркера, его секреты, база, egress | **чарт `domainscope` или Compose** | `values.yaml` / `.env` |
| Автоматическая связка воркера с проектом при установке всей платформы | **зонтичный чарт `hub-platform`** | `global.defaultProject.*`, секреты создаются сами |

Порядок работ:

```mermaid
flowchart TD
    A["Шаг 1 — В веб-интерфейсе:<br/>проект, продукты,<br/>учётная запись, права"]
    B["Шаг 2 — В веб-интерфейсе:<br/>заполнить периметр"]
    C["Шаг 3 — В воркере:<br/>переменные окружения,<br/>чарт или Compose"]
    D["Шаг 4 — Проверка связки:<br/>curl, логи, интерфейс"]
    E["Шаг 5 — Эксплуатация:<br/>разбор предложений,<br/>правка периметра"]

    A --> B --> C --> D --> E
    E -->|"периметр меняется<br/>без перезапуска воркера"| B

    classDef web fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#111827
    classDef worker fill:#dcfce7,stroke:#16a34a,stroke-width:2px,color:#111827
    classDef check fill:#fef9c3,stroke:#ca8a04,stroke-width:2px,color:#111827
    class A,B,E web
    class C worker
    class D check
```

## Шаг 1. В веб-интерфейсе: проект, продукты, доступ
...
Пошаговая инструкция по созданию проекта и продукта, если делаете это впервые:
...
### 1.3. Сервисная учётная запись

Раздел **API-аккаунты** в левом меню → **Создать Service Account**. Имя вида
`domainscope-perimeter`, описание — чтобы через полгода было понятно, чей это
ключ.

![Список сервисных учётных записей](assets/img/perimeter/sa-list-light.jpg#only-light)
![Список сервисных учётных записей](assets/img/perimeter/sa-list-dark.jpg#only-dark)

В строке учётной записи четыре действия: карандаш — переименовать, ключ —
управление ключами API, замок — права, корзина — удалить.

### 1.4. Ключ API

Иконка ключа в строке учётной записи → поле имени, срок действия в днях →
**Создать ключ**.

![Управление ключами API сервисной учётной записи](assets/img/perimeter/sa-keys-light.jpg#only-light)
![Управление ключами API сервисной учётной записи](assets/img/perimeter/sa-keys-dark.jpg#only-dark)

!!! danger "Полный ключ показывается один раз"
    Скопируйте его сразу — в списке остаётся только префикс вида `sa_a7ab5971_**`
    для аудита, сам ключ хранится в базе хешем и восстановлению не подлежит.
    Потеряли — выпускайте новый и отзывайте старый.

Формат ключа — `sa_<8 символов>_<секрет>`. Держать два действующих ключа
(`primary` и `backup`, как на снимке) удобно для ротации без простоя.

Подробнее про учётные записи и ключи:
[Загрузка отчётов внешних сканеров](integration-sarif.md).

### 1.5. Права

Иконка замка в строке учётной записи. В верхней строке формы: **тип ресурса**
(проект или продукт) → **выбор ресурса** → **право** → **Добавить**. Ниже —
список уже выданных прав.

![Права сервисной учётной записи](assets/img/perimeter/sa-permissions-light.jpg#only-light)
![Права сервисной учётной записи](assets/img/perimeter/sa-permissions-dark.jpg#only-dark)
...
**Достаточно одной записи: тип ресурса «Проект», ваш проект периметра, право
`upload_report`.** Права наследуются вниз: такая запись авторизует и загрузку
отчётов во все продукты проекта, и все четыре эндпоинта записи периметра. На
снимке выше показан обратный вариант — права выданы поштучно на каждый продукт;
он тоже рабочий, но требует правки при каждом новом продукте.
...
## Шаг 2. В веб-интерфейсе: периметр сканирования

**Проект → Скоуп** (`/projects/<идентификатор>/scope`). Три вкладки верхнего
уровня — «Скоуп», «Синхронизации», «Инвентарь»; внутри «Скоупа» — подвкладки с
счётчиками.
...
### 2.1. Как выглядят записи

Вкладка «Адреса» — подсети и одиночные адреса. Дочерние записи показываются с
отступом под своей родительской подсетью, колонка «Резолв» показывает, сколько
доменов резолвится в этот адрес, колонка «Источник» — откуда запись взялась
(`manual` — руками, `netbox_import` — из синхронизации, `approved_proposal` —
одобренное предложение сканера).

![Вкладка «Адреса» — подсети периметра](assets/img/perimeter/scope-cidr-light.jpg#only-light)
![Вкладка «Адреса» — подсети периметра](assets/img/perimeter/scope-cidr-dark.jpg#only-dark)

Вкладка «Домены» устроена так же. На снимке видна типовая конструкция: корневой
домен включён, поддомен `*.api` включён отдельной записью, а `mail` —
исключён.

![Вкладка «Домены» — домены периметра](assets/img/perimeter/scope-domains-light.jpg#only-light)
![Вкладка «Домены» — домены периметра](assets/img/perimeter/scope-domains-dark.jpg#only-dark)
...
Кнопка **Добавить запись** справа над таблицей. Тип (`cidr` или `domain`)
подставляется автоматически по мере ввода значения — отдельно выбирать его не
нужно. Обязательны только значение и действие; примечание, описание и теги —
по желанию, теги потом используются для обогащения находок.

![Добавление записи в периметр](assets/img/perimeter/scope-add-entry-light.jpg#only-light)
![Добавление записи в периметр](assets/img/perimeter/scope-add-entry-dark.jpg#only-dark)

#### 2.3. Импортом CSV

Кнопка **Импорт CSV** — для первичного наполнения из выгрузки IPAM или таблицы.
Содержимое вставляется прямо в поле. Заголовок обязателен, порядок колонок
любой:
...
![Импорт периметра из CSV](assets/img/perimeter/scope-csv-light.jpg#only-light)
![Импорт периметра из CSV](assets/img/perimeter/scope-csv-dark.jpg#only-dark)
...
#### 2.4. Синхронизацией с NetBox

Вкладка **Синхронизации**. Каждое задание — это отдельный периодический импорт
подсетей, адресов и DNS-зон из своего NetBox. В списке видно адрес, действие
(включать или исключать импортируемое), статус последнего запуска и
переключатель активности; иконки справа — запустить сейчас, изменить, удалить.

![Список заданий синхронизации с NetBox](assets/img/perimeter/scope-sync-list-light.jpg#only-light)
![Список заданий синхронизации с NetBox](assets/img/perimeter/scope-sync-list-dark.jpg#only-dark)

Форма создания — **Добавить синхронизацию**. Обязательны название, адрес NetBox,
токен и действие; ниже — фильтры: тип сетей (публичные — внешний периметр,
приватные — внутренний), арендаторы, теги на включение и исключение, DNS view,
интервал в часах и список статусов NetBox, которые считаются активными.

![Форма создания задания синхронизации](assets/img/perimeter/scope-sync-form-light.jpg#only-light)
![Форма создания задания синхронизации](assets/img/perimeter/scope-sync-form-dark.jpg#only-dark)

Токен NetBox хранится в базе Hub и используется воркером синхронизации — это
токен **Hub к NetBox**, не путать с ключом сервисной учётной записи для
DomainScope. Подробнее про фильтры, права токена и защиту от обращений во
внутреннюю сеть: [Синхронизация периметра с NetBox](integration-netbox.md).

#### 2.5. Предложениями самого сканера

После первого цикла обнаружения — вкладка «Предложения», см. раздел
«Предложения расширить периметр» ниже.
...
## Шаг 3. В воркере: переменные окружения

Всё, что ниже, задаётся **не в веб-интерфейсе**, а на стороне DomainScope: в
`.env` при запуске через Compose, в `values.yaml` чарта при запуске в
Kubernetes, либо в YAML-конфиге воркера. Приоритет: переменные окружения
перекрывают YAML, YAML перекрывает значения по умолчанию.
...
В чарте те же самые переменные задаются не напрямую, а через значения —
шаблон разворачивает их в переменные окружения контейнера. Соответствие:

| Значение чарта | Переменная воркера |
| --- | --- |
| `domainscope.env.hubEnabled` | `DOMAINSCOPE_HUB_ENABLED` |
| `domainscope.env.hubApiEndpoint` | `DOMAINSCOPE_HUB_API_ENDPOINT` |
| `domainscope.env.sarifEnabled` | `DOMAINSCOPE_SARIF_ENABLED` |
| `domainscope.env.sarifAutoUpload` | `DOMAINSCOPE_SARIF_AUTO_UPLOAD` |
| `domainscope.env.sarifApiEndpoint` | `DOMAINSCOPE_SARIF_API_ENDPOINT` |
| `domainscope.env.sarifProductId` | `DOMAINSCOPE_SARIF_PRODUCT_ID` |
| `domainscope.env.sarifIpScope` | `DOMAINSCOPE_SARIF_IP_SCOPE` |
| `domainscope.env.domains` | `DOMAINSCOPE_DOMAINS` |
| `domainscope.env.nucleiEnabled` и подобные | `DOMAINSCOPE_NUCLEI_ENABLED` и подобные |
| `domainscope.env.timeLoopDiscovery`, `timeLoopPortscan`, … | `DOMAINSCOPE_TIME_LOOP_*` |
| `secrets.*` (секрет чарта) | токены и пароли — через `secretKeyRef`, не открытым текстом |

Секреты (`hubApiToken`, `hubProjectId`, `sarifApiToken`, `postgresPassword`,
`dsn`) в значениях открытым текстом не задаются: чарт берёт их из секрета
Kubernetes.
...
Вкладка **Предложения** показывает, что именно предложено, каким сканером и
когда. Галочка справа — одобрить, крестик — отклонить; переключатель сверху
оставляет в списке только те предложения, которые касаются ранее отключённых
записей.

![Вкладка «Предложения» — предложения сканера](assets/img/perimeter/scope-proposals-light.jpg#only-light)
![Вкладка «Предложения» — предложения сканера](assets/img/perimeter/scope-proposals-dark.jpg#only-dark)

Администратор выбирает одно из трёх:

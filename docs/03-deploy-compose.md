...
Версия образов задаётся через `${HUB_VERSION:-0.31}` (backend/worker/frontend) и `${DS_VERSION:-0.31}` (domainscope). По умолчанию подтягивается тег `0.31`; задайте `HUB_VERSION`/`DS_VERSION` в `.env`, чтобы зафиксировать другую версию.

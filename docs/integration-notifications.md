...
### 2. Разрешить обращение к вашему серверу

**Обязательный шаг, без которого доставки не будет.** Плагину разрешено
обращаться только к адресам из списка. У остальных каналов адрес облака
известен заранее и зашит в описание плагина, а Mattermost вы размещаете у
себя — поэтому его адрес нужно назвать явно:

```ini
PLUGIN_EGRESS_ALLOWLIST=mattermost.example.com
```

Переменная задаётся для backend и требует перезапуска. Без неё установка
пройдёт, настройки сохранятся, а доставка будет отклоняться с сообщением
`host … not in egress allowlist for plugin` — то есть выглядеть как «плагин
не работает», хотя дело в разрешении.

Ограничение снимется, когда адрес из настроек проекта начнёт учитываться сам
(SecurityScanHub#569).

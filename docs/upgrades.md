...
Текущая опубликованная версия — **0.33** у всех компонентов: backend, worker,
frontend, сканер инфраструктурного кода и DomainScope. Версии при этом
независимые, и совпадение номеров — не правило: полный набор образов есть не
для каждой версии.
...
[Что нового в 0.33](#что-нового-в-033--hub).
...
- **Suspended-домены**: запрет по специфичности и зонд возврата — зонд
  отчитывается в начале цикла и не ждёт перечисления поддоменов.
- **Цели nuclei и ZAP проверяются скоупом Hub**, а не только владением
  адресом: прежде сканировалось и то, что из скоупа уже убрали.

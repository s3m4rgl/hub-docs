...
| Когда используется | Обогащение, источники находок, **провайдеры задач** | Каналы уведомлений |
...
!!! warning "Нативный плагин исполняется вне песочницы WASM"
    ...
    подключён, а нативные плагины (все шесть каналов уведомлений) видны, но
    **не ставятся**, пока источнику не выдано разрешение. Отказ не молчаливый:
    такие записи в каталоге помечены, кнопка установки выключена, а карточка
    объясняет причину и куда нажать.

    Обратное правило тоже жёсткое: **провайдер задач не может быть нативным**.
    Смысл этой возможности в том, чтобы адаптер под свою систему учёта написал
    сам заказчик, и безопасно это ровно потому, что такой плагин исполняется в
    песочнице WASM. Запрет проверяется и при установке, и при вызове — плагин,
    установленный до появления правила, всё равно не будет вызван.
...
[переменных Hub](env-hub.md#платформа-плагинов).

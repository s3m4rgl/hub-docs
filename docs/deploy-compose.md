...
Версия образов задаётся переменными `HUB_VERSION` (backend, worker, frontend) и `DS_VERSION` (DomainScope) в файле `.env`. По умолчанию используется `0.30` — последняя версия, для которой опубликованы образы всех компонентов. Перед сменой версии убедитесь, что нужный тег существует у каждого компонента: см. [Обновления](upgrades.md#выбор-версии).

...
Версия образов задаётся переменными `HUB_VERSION` (backend, worker, frontend) и `DS_VERSION` (DomainScope) в файле `.env`. По умолчанию обе — `0.33`. Совпадение версий не гарантировано: компоненты выпускаются независимо, и тег, существующий у Hub, может отсутствовать у DomainScope — тогда сервис получает `ImagePullBackOff` уже после старта остальных. Перед сменой версии убедитесь, что нужный тег существует у каждого компонента: см. [Обновления](upgrades.md#выбор-версии).
...
(локально — `http://localhost:8082/swagger/index.html`). Удобно для отладки интеграций и проверки контрактов. Портируемая спецификация OpenAPI 3 лежит в поставке — по ней генерируются клиенты.

### Метрики

Каждый долгоживущий процесс отдаёт метрики Prometheus на отдельном порту
(`METRICS_PORT`, по умолчанию `9090`). На хост он **намеренно не
пробрасывается** — читайте изнутри контейнера:

```bash
docker compose exec backend wget -qO- http://127.0.0.1:9090/metrics
```

### Образ frontend работает от непривилегированного пользователя

С 0.33 контейнер запускается под uid/gid `101` и пишет pid-файл в `/tmp`.
Дополнительных действий это не требует; проверьте только, если вы
переопределяли пользователя или монтировали тома с расчётом на root.

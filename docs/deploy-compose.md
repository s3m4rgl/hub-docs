...
Версия образов задаётся переменными `HUB_VERSION` (backend, worker, frontend) и `DS_VERSION` (DomainScope) в файле `.env`. По умолчанию `HUB_VERSION` — `0.33`, `DS_VERSION` — `0.32`. Номера не обязаны совпадать: у компонентов свои циклы выпуска, и последняя опубликованная версия DomainScope может отставать от версии Hub. Перед сменой версии убедитесь, что нужный тег существует у каждого компонента: см. [Обновления](upgrades.md#выбор-версии).

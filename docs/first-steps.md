...
flowchart TD
    A["Шаг 1 — Войти"]
    B["Шаг 2 — Создать проект"]
    C["Шаг 3 — Создать продукт"]
    D["Шаг 4 — Завести сервисную<br/>учётную запись и ключ"]
    E["Шаг 5 — Загрузить первый отчёт"]
    F["Шаг 6 — Убедиться, что находки появились"]
    G["Шаг 7 — Настроить сроки,<br/>уведомления, доступ"]
...
Чтобы завести первого администратора, **не выключая SSO**, перечислите его
адрес в `BOOTSTRAP_ADMIN_EMAILS` (через запятую, регистр не важен): при входе
через провайдера такой пользователь получает роль администратора. Адрес,
который провайдер не подтвердил, роли не получает.

Переменная выдаёт административные права, поэтому Hub при старте пишет об этом
предупреждение. **Уберите её**, как только администратор заведён и раздал
права остальным.
...
| Уведомления | Иначе о находках узнают, только зайдя в интерфейс. Каналы ставятся плагинами | [Уведомления](integration-notifications.md) |
| Учёт задач | Чтобы работа попадала исполнителю: Jira, GitHub Issues, Trello | [Учёт задач](integration-jira.md) |
| Плагины | Каналы уведомлений, провайдеры задач и обогащение ставятся плагинами | [Плагины](plugins.md) |

...
Если у закрываемой находки есть связанная задача в трекере, Hub отражает
закрытие через плагин-провайдера (см. [Учёт задач](integration-jira.md)):
...
- [Учёт задач](integration-jira.md) — настройки `auto_verify_close_comment` и `auto_verify_close_transition`

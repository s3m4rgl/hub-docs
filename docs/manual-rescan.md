...
- при заданном `auto_verify_close_transition` переводит задачу в указанный статус. Настраивается в **конфигурации плагина учёта задач для этого проекта** (правка проекта → учёт задач); в 0.33 эти поля переехали туда из прежнего `jira_config`:
